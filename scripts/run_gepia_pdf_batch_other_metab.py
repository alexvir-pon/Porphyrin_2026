...
def build_tasks(phases: Iterable[str], cancers=None) -> List[Dict]:
    ...
    # ── Pathway control datasets ──────────────────────────────────────────────
    # Matched cancer-normal pairs (n>=20 normal, Phase 5 cancers)
    MATCHED_CANCERS = [
        "BRCA_Tumor","COAD_Tumor","HNSC_Tumor","KICH_Tumor","KIRC_Tumor",
        "KIRP_Tumor","LIHC_Tumor","LUAD_Tumor","LUSC_Tumor","PRAD_Tumor",
        "STAD_Tumor","THCA_Tumor","UCEC_Tumor",
    ]
    MATCHED_NORMALS = [
        "BRCA_Normal","COAD_Normal","HNSC_Normal","KICH_Normal","KIRC_Normal",
        "KIRP_Normal","LIHC_Normal","LUAD_Normal","LUSC_Normal","PRAD_Normal",
        "STAD_Normal","THCA_Normal","UCEC_Normal",
    ]

    # Phase 7 — Fatty acid synthesis (chromatin-regulated control)
    # BRD4/ATAD2 documented to regulate FASN and lipogenic genes
    FA_PAIRS = [
        ("ACACA", "FASN"),
        ("FASN",  "SCD"),
        ("SCD",   "ELOVL6"),
    ...
    # Phase 8 — TCA cycle (non-chromatin-regulated control)
    # Regulated by substrate availability and allosteric feedback
    TCA_PAIRS = [
        ("CS",    "ACO2"),
        ("ACO2",  "IDH2"),
        ("IDH2",  "OGDH"),
    ...
    if "7" in phases:
        # 7A: fatty acid synthesis — matched cancer tissues
        selected = cancers or MATCHED_CANCERS
        for ds in selected:
            for g1, g2 in FA_PAIRS:
                tasks.append(dict(phase="7A", analysis="fa_within_cancer",
                                  gene1=g1, gene2=g2, dataset_override=ds))
        # 7B: fatty acid synthesis — matched normal tissues
        normals = MATCHED_NORMALS
        for ds in normals:
            for g1, g2 in FA_PAIRS:
                tasks.append(dict(phase="7B", analysis="fa_within_normal",
                                  gene1=g1, gene2=g2, dataset_override=ds))

    if "8" in phases:
        # 8A: TCA cycle — matched cancer tissues
        selected = cancers or MATCHED_CANCERS
        for ds in selected:
            for g1, g2 in TCA_PAIRS:
                tasks.append(dict(phase="8A", analysis="tca_within_cancer",
                                  gene1=g1, gene2=g2, dataset_override=ds))
        # 8B: TCA cycle — matched normal tissues
        normals = MATCHED_NORMALS
        for ds in normals:
            for g1, g2 in TCA_PAIRS:
                tasks.append(dict(phase="8B", analysis="tca_within_normal",
                                  gene1=g1, gene2=g2, dataset_override=ds))
...
def main():
    ...
    ap.add_argument("--phase", action="append", choices=["1", "2", "3", "5", "6", "7", "8"])
    ...
    phases = ["1", "2", "3", "5", "6", "7", "8"] if args.all else (args.phase or ["2"])
